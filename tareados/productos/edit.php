...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $sql = "UPDATE dbo.Productos
                SET CodigoInterno = ?, Nombre = ?, Costo = ?, Existencias = ?, CategoriaId = ?, BodegaId = ?
                WHERE IdProducto = ?";
        $params = [$codigo, $nombre, $costo, $existencias, $categoria_id, $bodega_id, $id];
...
<div class="container_CPB">
    <h1 class="mb-4 text-primary">Editar Producto</h1>
    ...
    <form method="post" novalidate>
        <div class="mb-3">
            ...
            <input type="text"
                   ...
                   value="<?= htmlspecialchars($_POST['codigo_interno'] ?? $producto['CodigoInterno']) ?>">
        ...
        <div class="mb-3">
            ...
            <input type="number" step="0.01" min="0"
                   ...
                   value="<?= htmlspecialchars($_POST['costo'] ?? $producto['Costo']) ?>">
        ...
        <div class="mb-3">
            ...
            <input type="number" min="0" step="1"
                   ...
                   id="existencias" name="existencias" required
                   value="<?= htmlspecialchars($_POST['existencias'] ?? $producto['Existencias']) ?>">
            <div class="invalid-feedback">Las existencias deben ser un número entero positivo.</div>
        </div>

        <div class="mb-3">
            <label for="categoria_id" class="form-label">Categoría</label>
            <select id="categoria_id" name="categoria_id" class="form-select" aria-label="Categoría del producto">
                <option value="">-- Sin categoría asignada --</option>
                <?php foreach ($categorias as $cat): ?>
                    <?php 
                        $selectedValue = $_POST['categoria_id'] ?? $producto['CategoriaId'] ?? '';
                        $selected = ($selectedValue == $cat['IdCategorias']) ? 'selected' : '';
                    ?>
                    <option value="<?= $cat['IdCategorias'] ?>" <?= $selected ?>>
        ...
        <div class="mb-4">
            ...
            <select id="bodega_id" name="bodega_id" class="form-select" aria-label="Bodega del producto">
                ...
                <?php foreach ($bodegas as $bod): ?>
                    <?php 
                        $selectedValue = $_POST['bodega_id'] ?? $producto['BodegaId'] ?? '';
                        $selected = ($selectedValue == $bod['IdBodegas']) ? 'selected' : '';
                    ?>
                    <option value="<?= $bod['IdBodegas'] ?>" <?= $selected ?>>
...
<script>
// Autofocus en el primer campo con error para mejor UX
